...
class Main:
    ...
    def crawl_product_set(self, node, max_item_num, min_price):
        amazon_api = AmazonAPI(
            ...
            settings.amazon_assosiate_id, 'JP'
        )
        product_set = set()
        now_price = min_price
        request_min_price = 0
        ...
        for i in range(request_set_num):
            ...
            for page in range(1, 11):
                self.logger.info(f'request node={node}, count={i * 10 + page}, min_price={request_min_price}, item_page={page}')
                products = amazon_api.search_products(
                    browse_node=node, item_page=page,
                    min_price=request_min_price, sort_by='Price:LowToHigh'
                )
    ...
    def out_product_set(self, product_set, outfile):
        with open(outfile, 'w', encoding='utf-8') as out:
            ...
            for x in sorted(product_set):
                out.write(f'"{x.authors[0]}","{x.series_title}","{x.title}","{x.price_display}","{x.url}","{x.genles}","{x.simhash().value}"\n')

    def main(self, args):
        self.logger.info(f'{args}')
        product_set = self.crawl_product_set(args.arg1, settings.max_item_num, int(args.min_price))
...
if(__name__ == '__main__'):
    ...
    parser.add_argument('--min_price', help='Min Price', default=str(settings.min_price))
